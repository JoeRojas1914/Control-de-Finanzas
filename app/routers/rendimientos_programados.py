...
from app.models import RendimientoProgramado, HorarioRendimiento, RendimientoDiario, Cuenta
...
def crear(datos: RendProgramadoCreate, db: Session = Depends(get_db)):
...
@router.patch("/{rp_id}")
def editar(rp_id: int, datos: RendProgramadoCreate, db: Session = Depends(get_db)):
    rp = db.query(RendimientoProgramado).filter(RendimientoProgramado.id == rp_id).first()
    if not rp:
        raise HTTPException(status_code=404, detail="No encontrado")
    if not db.query(Cuenta).filter(Cuenta.id == datos.cuenta_id).first():
        raise HTTPException(status_code=404, detail="Cuenta no encontrada")
...
def toggle(rp_id: int, db: Session = Depends(get_db)):
    rp = db.query(RendimientoProgramado).filter(RendimientoProgramado.id == rp_id).first()
    if not rp:
        raise HTTPException(status_code=404, detail="No encontrado")
...
@router.delete("/{rp_id}")
def eliminar(rp_id: int, db: Session = Depends(get_db)):
    rp = db.query(RendimientoProgramado).filter(RendimientoProgramado.id == rp_id).first()
    if not rp:
        raise HTTPException(status_code=404, detail="No encontrado")
    db.delete(rp)
...
def crear_horario(datos: HorarioCreate, db: Session = Depends(get_db)):
    if not db.query(Cuenta).filter(Cuenta.id == datos.cuenta_id).first():
        raise HTTPException(status_code=404, detail="Cuenta no encontrada")
    h = HorarioRendimiento(**datos.model_dump())
    db.add(h)
    db.commit()
    db.refresh(h)
...
def editar_horario(h_id: int, datos: HorarioCreate, db: Session = Depends(get_db)):
    h = db.query(HorarioRendimiento).filter(HorarioRendimiento.id == h_id).first()
    if not h:
        raise HTTPException(status_code=404, detail="No encontrado")
    if not db.query(Cuenta).filter(Cuenta.id == datos.cuenta_id).first():
        raise HTTPException(status_code=404, detail="Cuenta no encontrada")
...
def toggle_horario(h_id: int, db: Session = Depends(get_db)):
    h = db.query(HorarioRendimiento).filter(HorarioRendimiento.id == h_id).first()
    if not h:
        raise HTTPException(status_code=404, detail="No encontrado")
...
def eliminar_horario(h_id: int, db: Session = Depends(get_db)):
    h = db.query(HorarioRendimiento).filter(HorarioRendimiento.id == h_id).first()
    if not h:
        raise HTTPException(status_code=404, detail="No encontrado")
    db.delete(h)
...
@router.post("/aplicar")
def aplicar(db: Session = Depends(get_db)):
    ...
    for rp in activos:
        ...
        while rp.proxima_fecha.date() <= hoy and iteraciones < 730:
            rend = RendimientoDiario(
                cuenta_id = rp.cuenta_id,
                monto     = rp.monto,
                fecha     = rp.proxima_fecha,
            )
            db.add(rend)
    ...
    # Horario semanal
    horarios = db.query(HorarioRendimiento).filter(HorarioRendimiento.activo == True).all()
    for h in horarios:
        ...
        montos_semana = [h.lunes, h.martes, h.miercoles, h.jueves, h.viernes, h.sabado, h.domingo]
        while dia <= hoy:
            monto = montos_semana[dia.weekday()]
